...
// Helper to create ChartJS configuration
...
async function main() {
    ...
    const { text: marketTrends, sources } = await generateText({
        ...
        prompt: `Search the web for market trends for bitcoin in North America for 2024-2025.
    ...
    const { object: chartData } = await generateObject({
        ...
        schema: z.object({
            chartConfigurations: z
                ...
                colors: z
                    .array(z.string())
                    .describe('A list of colors to use for the chart, e.g. "rgba(255, 99, 132, 0.8)"'),
            }))
...
`,
    ...
    // Step 3: generate final HTML report and save as pdf
    ...
    **Data:**
    ...
    // Stripe Markdown from HTML code block
    const finalHtml = htmlReport.replace(/^```html\n/, "").replace(/\n```$/, "");
    const browser = await puppeteer.launch();
    const page = await browser.newPage();
    await page.setContent(finalHtml);
    await page.pdf({ path: "market_analysis_report.pdf", format: "A4" });
    await browser.close();
